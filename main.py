...
async def api_wiener_afterhours(request: Request, fra: str = "2026-09-01"):
    ...
    if request.query_params.get("secret") != WEBHOOK_SECRET:
        _kræv_login(request)
    return database.hent_wiener_afterhours(fra)
...
async def api_diag_raw_salg2(request: Request, q: str = "", dage: int = 30, secret: Optional[str] = None):
    """MIDLERTIDIG: rå salg pr. varenavn+varenummer (seneste N dage) — find SKU."""
    if secret != WEBHOOK_SECRET:
        _kræv_login(request)
    import sqlite3 as _sq
    from datetime import date as _d, timedelta as _td
    fra = (_d.today() - _td(days=dage)).isoformat()
    con = _sq.connect(database.DB_PATH); con.row_factory = _sq.Row
    rows = con.execute(
        "SELECT varenavn, CAST(CAST(varenummer AS REAL) AS INTEGER) AS vn, SUM(antal) AS ant "
        "FROM transaktioner WHERE dato>=? GROUP BY varenavn, vn ORDER BY varenavn", (fra,)).fetchall()
    con.close()
    ql = q.lower()
    return {"salg": [{"navn": r["varenavn"], "vn": r["vn"], "ant": int(r["ant"] or 0)}
...
@app.get("/api/bagvaerk/optimering")
async def api_bagvaerk_optimering(request: Request, antal: int = 12):
    """Newsvendor-anbefaling pr. vare: service level + retur-type → bestillingsstrategi."""
    _kræv_login(request)
...
async def prisaendring_bulk(request: Request):
    ...
    if not linjer:
        raise HTTPException(status_code=400, detail="Ingen linjer")
    antal = database.gem_prisaendringer_bulk(linjer, kilde=body.get("kilde", "manuel"))
    return {"ok": True, "linjer": antal}
...
@app.post("/api/db-shopbox/levering-override")
async def api_db_levering_override(request: Request):
    """Skift levering til/fra for én dag i DB-Shopbox (175 kr, standard fre-søn).
    Body: {dato}. Ét klik flipper; næste klik nulstiller til auto. Login/secret."""
    try:
        body = await request.json()
    except Exception:
        body = {}
    ...
    if not dato:
        raise HTTPException(status_code=400, detail="Mangler dato")
    return database.toggle_db_levering_override(str(dato)[:10])
...
async def bageri_vare_opslag(request: Request, q: str = "", uger: int = 12,
                             alle: bool = False, secret: Optional[str] = None):
    ...
    Kun Bagværk-kategorien som standard (så fx 'Kødboller i tomat' ikke matcher
    'bolle'); ?alle=true for at søge på tværs af alle kategorier.
    Login eller webhook-secret."""
    if request.headers.get("X-Webhook-Secret") != WEBHOOK_SECRET and secret != WEBHOOK_SECRET:
        _kræv_login(request)
    from datetime import date, timedelta
    ...
    like = f"%{q.lower()}%"
    kat_filter = "" if alle else "AND TRIM(kategori) = 'Bagværk'"
    with database._conn() as conn:
        rows = conn.execute(f"""
            SELECT CAST(CAST(varenummer AS REAL) AS INTEGER) AS vn,
                   ...
                   MAX(dato) AS sidst
            FROM transaktioner
            WHERE dato >= ? AND LOWER(varenavn) LIKE ? {kat_filter}
            GROUP BY vn, varenavn
            ORDER BY stk DESC
        """, (fra, like)).fetchall()
    return {"q": q, "uger": uger, "kun_bagvaerk": not alle, "antal": len(rows),
